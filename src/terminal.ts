...
function footer(state: AppState): string[] {
  return [
    ...
    `${gray("↑/↓ select trader  ·  space copy paper  ·  c follow  ·  p pause  ·  r refresh  ·  q quit")} ${state.paused ? yellow("PAUSED") : green("LIVE")}`
  ];
}
...
export class TerminalApp {
  ...
  private render(): void {
    ...
    const bottomPanels = joinColumns([
      ...
      box("TOP NARRATIVES", Math.floor(right * 0.48), narrativeRows(), colors.yellow),
